...
def plot_security(df: pd.DataFrame, mnemonic: str, metric: Union[str, dict], 
                  kind: str=['line', 'ohlc', 'candlestick'], 
                  resample_freq: str='1Min', volume: bool=False):
    ...
    if kind=='line':
        ...
        trace1 = go.Ohlc(x=selected.index,
        ...
        trace1 = go.Candlestick(x=selected.index,
                    open=selected['StartPrice'],
                    high=selected['MaxPrice'],
                    low=selected['MinPrice'],
                    close=selected['EndPrice'], 
    ...
    trace2 = go.Bar(x=selected.index, 
